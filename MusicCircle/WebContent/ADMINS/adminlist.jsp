...
<body>
	...
	<a href="adminuseradd"  class="btn button_btn bg-deep-blue" title="添加管理员"><i class="fa  fa-edit"></i>&nbsp;添加管理员</a>
	  <!-- <select class="select Competence_sort" name="admin-role" size="1" id="Competence_sort">
			...
			<%-- <span>${umuser.admname}为禁用状态</span> --%>
	       ...
	       <table id="sample_table" class="table table_list table_striped table-bordered dataTable no-footer">
			 <thead>
				<tr>
	             ...
	             <tbody>
	             	<c:forEach items="${adminList}" var="lists">
	             		<tr>
						<td>${lists.admname}</td>
						<td>${lists.admtimes}</td>
						<td>${lists.admlasts}</td>
						<td>
							<a onClick="member_stop(this,'10001')"  href="usermenupage?userid=${lists.admid}" title="权限设置"  class="btn btn-xs btn-status">设置</a>
						</td>
		                <td class="td-status">
		                	<c:if test="${lists.admstate eq 1}"><span class="label label-success label-sm">已启用</span></c:if>
		                	<c:if test="${lists.admstate eq 0}"><span class="label label-fail label-sm">已禁用</span></c:if>
		                </td>
						<td class="td-manage">
