...
<title>声音录制文案信息</title>
</head>
<body>
	<div class="margin Competence_style" id="page_style">
	    <div class="operation clearfix">
		<a href="voicesoffaddpage"  class="btn button_btn bg-deep-blue" title="添加声音录制文案"><i class="fa  fa-edit"></i>添加声音录制文案</a>
	       ...
	       <table id="sample_table" class="table table_list table_striped table-bordered dataTable no-footer">
			 <thead>
				<tr>
	             ...
	             <tbody>
	             	<c:forEach items="${voicesofferlist}" var="lists">
	             		<tr>
							<td>${lists.id}</td>
							<td>
								${lists.title}
							...
							<td>
								<c:if test="${lists.strus eq 1}">
									<span style="color:blue;">可用</span>
								</c:if>
								<c:if test="${lists.strus ne 1}">
									<span style="color:red;">不可用</span>
								</c:if>
							</td>								
							<td>
			                 <a title="编辑" href="updatevoicesoffpage?id=${lists.id}" class="btn button_btn bg-deep-blue">修改</a>        
			                 <a title="删除" href="voicesoffupdatestruts?id=${lists.id}&struts=0"class="btn button_btn btn-danger">删除</a>
							</td>
					   </tr>
	             	</c:forEach>
			      </tbody>
	...
	<script type="text/javascript" src="js/jquery.voiceofficialinfopagination.js"></script>
