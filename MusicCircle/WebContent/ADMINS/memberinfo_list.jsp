...
<body>
	       ...
	       <table id="sample_table" class="table table_list table_striped table-bordered dataTable no-footer">
			 <thead>
				<tr>
	             ...
	             <tbody>
	             	<c:forEach items="${memberlist}" var="lists">
	             		<tr>
							<td>${lists.uuid}</td>
							<td>${lists.megname}</td>
							<td>${lists.daysnum}</td>
							<td>${lists.begintime}</td>
							<td>${lists.endestime}</td>
							<td>
								<c:if test="${lists.strutus eq 1}"><span style="color:blue;">未到期</span></c:if>
								<c:if test="${lists.strutus eq 0}"><span style="color:red;">已到期</span></c:if>
							</td>
							<td>
								<c:if test="${lists.disabls eq 1}"><span style="color:blue;">未禁用</span></c:if>
								<c:if test="${lists.disabls eq 0}"><span style="color:red;">已禁用</span></c:if>
							</td>
						<td>
	...
	<script type="text/javascript" src="js/jquery.memberinfopagination.js"></script>
