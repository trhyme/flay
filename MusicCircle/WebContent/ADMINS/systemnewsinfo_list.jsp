...
<body>
	<div class="margin Competence_style" id="page_style">
		...
		<a href="systeminsertpage"  class="btn button_btn bg-deep-blue" title="系统消息发送"><i class="fa  fa-edit"></i>系统消息发送</a>
	       ...
	       <table id="sample_table" class="table table_list table_striped table-bordered dataTable no-footer">
			 <thead>
				<tr>
	             ...
	             <tbody>
	             	<c:forEach items="${systemnewslist}" var="lists">
	             		<tr>
							<td>${lists.title}</td>
							<td>
								${lists.infos}
							...
							<td>
								<a href="${lists.htmlurl}" target="_blank">
									<img alt="" src="${lists.imgurl}" style="width:50px;height:50px;">
								</a>
							</td>
							<td>
								<c:if test="${lists.stratus eq 1}">未删除</c:if>
								<c:if test="${lists.stratus eq 0}">已删除</c:if>
							</td>
							<td>${lists.addtime}</td>
						<td>
		                 <a title="编辑" href="updatetosystempage?id=${lists.id}" class="btn button_btn bg-deep-blue">编辑</a>        
		                 <c:if test="${lists.stratus eq 1}">
		                 	<a title="删除" href="deletesystemone?id=${lists.id}&stratus=0" class="btn button_btn btn-danger">删除</a>
		                 </c:if>
		                 <c:if test="${lists.stratus eq 0}">
		                 	<a title="恢复" href="deletesystemone?id=${lists.id}&stratus=0" class="btn button_btn btn-danger" style="background-color:yellow;">恢复</a>
		                 </c:if>
	...
	<script type="text/javascript" src="js/jquery.appgiftspagination2.js"></script>
