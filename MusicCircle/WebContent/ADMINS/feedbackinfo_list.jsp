	        ...
	        <table id="sample_table" class="table table_list table_striped table-bordered dataTable no-footer">
			 <thead>
				<tr>
	             ...
	             <tbody>
	             	<c:forEach items="${feedbacklist}" var="lists">
	             		<tr>
						<td>${lists.uuid}</td>
						<td>${lists.unames}</td>
						<td>${lists.uphone}</td>
						...
						<td>${lists.addtime}</td>
						<td>
							<c:if test="${lists.stratus eq 0}"><span style="color:red;">未处理</span></c:if>
							<c:if test="${lists.stratus eq 1}"><span style="color:blue;">已处理</span></c:if>
						</td>
						<td style="padding:3px;margin:auto;">
							<form action="updatefeedbackrep" method="post">
								<input type="hidden" name="thepage" value="${nowpage}" readonly="readonly">
								<span style="margin:auto;"><textarea rows="" cols="" name="replays">${lists.replyes}</textarea></span>
								<span><input type="submit" value="提 交" style="margin:auto;margin-top:-18px;"></span>
								<input type="hidden" name="feedid" value="${lists.id}" readonly="readonly">
							</form>
						</td>
						<td>${lists.uptimes}</td>
						<td>
							<a href="?id=${lists.id}" style="cursor:pointer;"><img src="images/report_do.png" id="${lists.id}" style="width:24px;"></a>
						</td>
...
function add_payment(index ){
	...
	<script type="text/javascript" src="js/jquery.feedbackinfopagination.js"></script>
